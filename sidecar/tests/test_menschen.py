...
# -- Die Naht zwischen Abgeben und Menschen ---------------------------------
#
# Beide Etappen sind getrennt entstanden: die Delegation kennt keine Menschen,
# die Personenebene kannte das Feld `wartet_auf` noch nicht. Genau an solchen
# Nähten reißt es, weil jede Seite für sich geprüft war.


def test_abgegebenes_landet_beim_menschen(
    episodes: EpisodeStore, tasks: TaskStore
) -> None:
    _episode(episodes, "Besprechung zum Vertrag.", ["Frau Becker"])
    aufgabe = tasks.add("Vergütungsspanne festlegen", _prov(SourceType.USER_STATED))
    tasks.warten_auf(aufgabe.id, "Frau Becker", at=JETZT)

    mensch = personen.eine("Frau Becker", episodes=episodes, tasks=tasks, jetzt=JETZT)

    assert [a["title"] for a in mensch.offene_aufgaben] == ["Vergütungsspanne festlegen"]


def test_das_feld_schlaegt_den_titel(episodes: EpisodeStore, tasks: TaskStore) -> None:
    """Eine Angabe schlägt eine Vermutung — sonst zählt eine abgegebene
    Aufgabe bei dem, der nur im Titel steht."""
    _episode(episodes, "Zum Vertrag.", ["Frau Becker", "Herr Ohlsen"])
    aufgabe = tasks.add("Rückfrage an Herr Ohlsen", _prov(SourceType.USER_STATED))
    tasks.warten_auf(aufgabe.id, "Frau Becker", at=JETZT)

    becker = personen.eine("Frau Becker", episodes=episodes, tasks=tasks, jetzt=JETZT)
    ohlsen = personen.eine("Herr Ohlsen", episodes=episodes, tasks=tasks, jetzt=JETZT)

    assert [a["title"] for a in becker.offene_aufgaben] == ["Rückfrage an Herr Ohlsen"]
    assert ohlsen.offene_aufgaben == []


def test_zurueckgeholtes_liegt_bei_niemandem_mehr(
    episodes: EpisodeStore, tasks: TaskStore
) -> None:
    _episode(episodes, "Zum Vertrag.", ["Frau Becker"])
    aufgabe = tasks.add("Vergütungsspanne festlegen", _prov(SourceType.USER_STATED))
    tasks.warten_auf(aufgabe.id, "Frau Becker", at=JETZT)
    tasks.zurueckholen(aufgabe.id)

    mensch = personen.eine("Frau Becker", episodes=episodes, tasks=tasks, jetzt=JETZT)

    assert mensch.offene_aufgaben == []
